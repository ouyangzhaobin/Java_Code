...
// 
//
// Related Topics 脑筋急转弯 数组 前缀和 排序 👍 21 👎 0
 ...
 * @title 移动机器人
 * @author ouyang
 * @date 2023-10-10 00:12:55
 */

class P_2731_MovementOfRobots{
	 public static void main(String[] args) {
	 	 //测试代码
	 	 Solution solution = new P_2731_MovementOfRobots().new Solution();
	 }
...
class Solution {
    ...
    public int sumDistance(int[] nums, String s, int d) {
		...
		for (int i = 1; i < length; i++) {
			res += 1L * (pos[i] - pos[i-1]) * i % MOD * (length - i) % MOD;
			res %= MOD;
		}
		return (int) res;
	}
